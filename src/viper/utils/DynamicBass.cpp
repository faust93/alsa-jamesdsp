...
void DynamicBass::FilterSamples(double *samplesL, double *samplesR, uint32_t size) {
    if (this->lowFreqX <= 120) {
        for (uint32_t i = 0; i < size; i++) {
            float left = samplesL[i];
            float right = samplesR[i];
            float avg = (float) this->lowPass.ProcessSample(left + right);
            samplesL[i] = left + avg;
            samplesR[i] = right + avg;
        }
    } else {
        for (uint32_t i = 0; i < size; i++) {
            float x1, x2, x3, x4, x5, x6, y1, y2, y3, y4, y5, y6;

            this->filterX.DoFilterLeft(samplesL[i], &x1, &x2, &x3);
            this->filterX.DoFilterRight(samplesR[i], &x4, &x5, &x6);
            this->filterY.DoFilterLeft(this->bassGain * x1, &y1, &y2, &y3);
            this->filterY.DoFilterRight(this->bassGain * x4, &y4, &y5, &y6);

            samplesL[i] = x2 + y3 + this->sideGainX * y2 + this->sideGainY * y1 + x3;
            samplesR[i] = x5 + y6 + this->sideGainX * y5 + this->sideGainY * y4 + x6;
        }
    }
}
