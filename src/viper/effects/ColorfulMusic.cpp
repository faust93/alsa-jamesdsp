...
void ColorfulMusic::Process(double *samplesL, double *samplesR, uint32_t size) {
    if (!this->enabled) return;

    this->depthSurround.Process(samplesL, samplesR, size);
    this->stereo3dSurround.Process(samplesL, samplesR, size);
}
