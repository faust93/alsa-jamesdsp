...
void AnalogX::Process(double *samplesL, double *samplesR, uint32_t size) {
    if (this->enable) {
        for (uint32_t i = 0; i < size; i ++) {
            double inL = samplesL[i];
...
//            this->freqRange += size;
//            memset(samplesL, 0, size * sizeof(double));
//            memset(samplesR, 0, size * sizeof(double));
//        }
    }
}
...
void AnalogX::Reset() {
    ...
    switch (this->processingModel) {
        ...
        case 3: {
            ...
            this->lowPass[0].RefreshFilter(MultiBiquad::FilterType::LOW_PASS, 0.1, 14391.0, this->samplingRate, 0.717, false);
            this->lowPass[1].RefreshFilter(MultiBiquad::FilterType::LOW_PASS, 0.1, 14391.0, this->samplingRate, 0.717, false);
            break;
        }
    }
